...
function mostrarsemana($data,$cod_sala,$qual_semana,$prox_semana,$dia_atual){ 
	...
	echo("Hoje � "); echo $dia78; echo(" de "); echo $nomes_meses["$mes78"]; echo(" de "); echo $ano78; echo(" ($tempo78)");
			...
			<TD >  <br> 
				...
				<select name="cod_sala" OnChange="javascript:alterarsala();">
					<?php
						...
						$result5 = mysqli_query($db,$sql5) or die(mysql_error());
				...
				if(($x>=1)&&($y>=1)){ // PREENCHER CELULAS DE RESERVA
					...
					if($linha["cod_sala"]==""){
						...
						echo("<TD bgcolor='#9FB6CD'><center>");
							...
							$resultado1 = mysqli_query($db,$sql1) or die(mysql_error());
...
if($enviar=="sim"){ // INICIO_ENVIAR_0
	...
	if(($linha1["matsup"]==$matricula)&&($linha1["senha"]==$senha)){ // REL_000
		...
		$resultado8 = mysqli_query($db,$sql8) or die(mysql_error());
		...
		if(($min_inicio==00)&&($tempo==0)){ // REF_001
			...
			if($linha["cod_sala"]==$cod_sala){
				$existe++;
				echo ("<html><SCRIPT LANGUAGE=\"JavaScript\"> function tela(){ alert('A dura��o de sua reserva esta confitante com outra. Por favor verifique.'); window.navigate('"); echo $PHP_SELF; echo("?codsala=$cod_sala&qualsemana=$qual_semana1&prox_semana=$prox_semana&dia_atual$dia_atual'); } </SCRIPT> <BODY OnLoad=\"javascript:tela();\" > </body></html>");
			}
		...
		echo ("<SCRIPT LANGUAGE=\"JavaScript\"> function tela(){ alert('Usuário ou senha incorreta!. Tente novamente.'); window.navigate('"); echo $PHP_SELF; echo("?codsala=$cod_sala&qualsemana=$qual_semana1&prox_semana=$prox_semana&dia_atual=$dia_atual'); } </SCRIPT> <BODY OnLoad=\"javascript:tela();\" > </body>");
	...
	if(($env_cod_sala)||($proxima)||($anterior)){ // REF_005
		...
		if($codsala){ // REF_006
			...
			//echo $tempo; echo(" - "); echo $dia; echo(" -- "); echo $mes; echo (" -- "); echo $ano; echo (" -- ");
			 //echo $tempo; ; echo $dia; echo $mes; echo $ano;
			...
			if($dia_da_semana=="Tue"){   $dia_da_semana="Ter�a";   $inicio=-1; $inicio2=-1; $iniciok=2; $inicio_prox=6; }
			...
			if($dia_da_semana=="Sat"){   $dia_da_semana="S�bado";  $inicio=-5; $inicio2=-5; $iniciok=6; $inicio_prox=2; }
